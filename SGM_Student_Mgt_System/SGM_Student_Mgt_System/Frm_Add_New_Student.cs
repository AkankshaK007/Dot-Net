...
namespace SGM_Student_Mgt_System
{
    public partial class Frm_Add_New_Student : Form
    {
        ...
        int Auto_Incr()
        {
            ...
            if (Cnt > 0)
            {
                Cmd.CommandText = "Select Max([Roll No])From Student_Details";
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            ...
            if (tb_Roll_No.Text != "" && tb_Name.Text != "" && tb_Mobile_No.Text != "" && cmb_Course.Text != "")
            {
                SqlCommand Cmd = new SqlCommand();
                Cmd.Connection = Con;
                Cmd.CommandText = "Insert Into Student_Details([Roll No],Name,DOB,[Mobile No],Course)Values(@RNo,@Name,@DOB,@MobNo,@Course)";
